...
const CSS = ''
  + '.cv-root{display:flex;height:100%;min-height:0;color:inherit;font-family:inherit}'
  ...
  + '.cvn-hint{font-size:10px;color:#7a8399;flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}'
...
function CanvasView(props) {
  ...
  function onExport(fmt) {
    ...
    patch(function (s) { return Object.assign({}, s, { exporting: true, exportMsg: '导出中…' }) })
    rpc('export', { path: state.picked, format: fmt, w: 1600, h: 1200, dir: state.saveDir }).then(function (res) {
      const ok = res !== null && res !== undefined && typeof res === 'object'
      const out = ok && typeof res.out === 'string' ? res.out : ''
      const err = ok && typeof res.error === 'string' ? res.error : 'unknown'
      patch(function (s) { return Object.assign({}, s, { exporting: false, exportMsg: out !== '' ? ('✓ 已导出 ' + out) : ('导出失败: ' + err) }) })
    }, function (e) {
      patch(function (s) { return Object.assign({}, s, { exporting: false, exportMsg: '导出失败: ' + String(e) }) })
    })
  }
  ...
  function pickSaveDir() {
    ...
    return rpc('list').then(function (res) {
      const root = res !== null && typeof res === 'object' && typeof res.root === 'string' ? res.root : ''
      patch(function (s) { return Object.assign({}, s, { picking: false, saveDir: root }) })
    }, function () {
...
function apply(ctx, env) {
  __ctx = ctx
  __styles = env !== undefined && env !== null ? env.styles : null
  const slots = ctx.get('slots')
  if (slots === undefined) {
    console.error('canvas: slots service unavailable')
    return
  }
  const disposeCss = __styles !== null ? __styles.insert(CSS) : () => {}
  ...
  ctx.effect(function () {
    return function () { disposeOverlay(); disposeReg(); disposeCss(); __ctx = null }
		...
		const inject = ["timer", "slots", "connection"];
		exports.apply = apply;
