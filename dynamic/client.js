  const CSS = '.cv-root{display:flex;height:100%;min-height:0;color:inherit;font-family:inherit}'
    ...
    + '.cv-stage{flex:1;min-height:0;overflow:auto;display:flex;justify-content:center}'
    + '.cv-stage-dark{background:#0e0f12}'
    + '.cv-stage-light{background:#fff}'
    + '.cv-holder{height:100%;flex:none;position:relative}'
    + '.cv-frame{position:absolute;inset:0;width:100%;height:100%;border:0;display:block}'
    ...
    + '.cvn-card.moving{cursor:grabbing;user-select:none}'
    + '.cvn-head{display:flex;align-items:center;gap:6px;padding:8px 10px 6px}'
    + '.cvn-grip{color:#b3bacb;font-size:13px;line-height:1;padding:0 2px}'
  ...
  function CanvasView(props) {
    ...
    function doList() {
      host.call('canvas/list').then(function (res) {
        const ok = res !== null && res !== undefined && typeof res === 'object'
    ...
    React.useEffect(function () {
      ...
      host.call('canvas/read', { path: state.picked }).then(function (res) {
        if (!alive) return
        const ok = res !== null && res !== undefined && typeof res === 'object'
        const errText = ok && typeof res.error === 'string' && res.error !== '' ? res.error : ''
    ...
    function pickSaveDir() {
      ...
      const workspaces = pluginCtx !== null ? pluginCtx.get('workspaces') : undefined
    ...
    function onExport(fmt) {
      ...
      if (state.saveDir !== '') req.dir = state.saveDir
      host.call('canvas/export', req).then(function (res) {
        const ok = res !== null && res !== undefined && typeof res === 'object'
        const out = ok && typeof res.out === 'string' ? res.out : ''
        const err = ok && typeof res.error === 'string' ? res.error : 'unknown'
        const ms = ok && typeof res.ms === 'number' ? (' · ' + String(res.ms) + 's') : ''
        patch(function (s) { return Object.assign({}, s, { exporting: false, exportMsg: out !== '' ? ('✓ 已导出 ' + out + ms) : ('导出失败: ' + err), exportOut: out !== '' && typeof res.outPath === 'string' ? res.outPath : '' }) })
      }, function (e) {
    ...
    function onExportAs() {
      ...
      pickSaveDir().then(function (dir) {
        if (dir === null) return
        patch(function (s) { return Object.assign({}, s, { exporting: true, exportMsg: '导出中…', exportOut: '' }) })
        const req = { path: state.picked, format: 'png', w: 1600, h: 1200 }
        if (dir !== '') req.dir = dir
        host.call('canvas/export', req).then(function (res) {
          const ok = res !== null && res !== undefined && typeof res === 'object'
          const out = ok && typeof res.out === 'string' ? res.out : ''
    ...
    function onReveal() {
      ...
      host.call('canvas/reveal', { path: state.exportOut }).then(function () {}, function () {})
    }
    ...
    else if (state.files.length === 0) sideNode = h('div', { className: 'cv-empty' }, '没有找到 HTML 文件。')
    ...
    if (state.picked === null || state.picked === undefined) {
      bodyNode = h('div', { className: 'cv-empty cv-center', style: { width: '100%' } }, '选择左侧文件预览')
    } else if (state.error !== '') {
    ...
    const saveDirLabel = state.saveDir !== '' ? state.saveDir : '默认（HTML 同目录）'
    ...
    return h('div', { className: 'cv-root' },
      ...
      h('div', { className: 'cv-main' },
        ...
        h('div', { className: 'cv-save-row' },
          ...
          h('span', { className: 'cv-save-path', title: saveDirLabel }, saveDirLabel),
  ...
  function NotifierCard(props) {
    const h = React.createElement
    const pair = React.useState({ open: false, path: null, name: '', html: '', expanded: false, exporting: false, msg: '', out: '' })
    const state = pair[0]
    const setState = pair[1]
    const posPair = React.useState(null)
    const pos = posPair[0]
    const setPos = posPair[1]
    const sizePair = React.useState({ w: 680, h: 460 })
    ...
    React.useEffect(function () {
      ...
      const tick = function () {
        ...
        host.call('canvas/list').then(function (res) {
          ...
          patch(function (s) { return Object.assign({}, s, { open: true, path: p, name: nm, html: '', expanded: false, msg: '', out: '' }) })
          host.call('canvas/read', { path: p }).then(function (r2) {
    ...
    function onExport(fmt) {
      ...
      patch(function (s) { return Object.assign({}, s, { exporting: true, msg: '导出中…', out: '' }) })
      host.call('canvas/export', { path: state.path, format: fmt, w: 1600, h: 1200 }).then(function (res) {
        const ok = res !== null && res !== undefined && typeof res === 'object'
        const out = ok && typeof res.out === 'string' ? res.out : ''
        const err = ok && typeof res.error === 'string' ? res.error : 'unknown'
        const ms = ok && typeof res.ms === 'number' ? (' · ' + String(res.ms) + 's') : ''
        patch(function (s) { return Object.assign({}, s, { exporting: false, msg: out !== '' ? ('✓ ' + out + ms) : ('失败: ' + err), out: out !== '' && typeof res.outPath === 'string' ? res.outPath : '' }) })
      }, function (e) {
        patch(function (s) { return Object.assign({}, s, { exporting: false, msg: '失败: ' + String(e), out: '' }) })
      })
    }
    function onReveal() {
      if (state.out === '') return
      host.call('canvas/reveal', { path: state.out }).then(function () {}, function () {})
    }

    function isFunctional(t) {
      if (t === null || t === undefined || typeof t.closest !== 'function') return false
      return t.closest('button, a, input, textarea, iframe, [data-cvdrag="size"]') !== null
    }

    function onPointerDown(e) {
      if (e.currentTarget === null || e.currentTarget === undefined) return
      if (e.button !== undefined && e.button !== 0) return
      const root = e.currentTarget
      const t = e.target
      const rsz = t !== null && typeof t.closest === 'function' && t.closest('[data-cvdrag="size"]') !== null
      if (!rsz && isFunctional(t)) return
      if (typeof root.setPointerCapture === 'function') {
        try { root.setPointerCapture(e.pointerId) } catch (err) { }
      }
      if (rsz) {
        setDrag({ mode: 'size', sx: e.clientX, sy: e.clientY, ox: 0, oy: 0, ow: size.w, oh: size.h, moved: false, onThumb: false })
        return
      }
      let r = { left: 0, top: 0 }
      ...
      const onThumb = t !== null && typeof t.closest === 'function' && !state.expanded
        ? t.closest('[data-cvthumb]') !== null
        : false
      setDrag({ mode: 'move', sx: e.clientX, sy: e.clientY, ox: r.left, oy: r.top, ow: 0, oh: 0, moved: false, onThumb: onThumb })
    }
    function onPointerMove(e) {
      ...
      if (!drag.moved && dx * dx + dy * dy > 16) {
        setDrag(Object.assign({}, drag, { moved: true }))
        return
      }
      if (drag.mode === 'move') {
    ...
    function onPointerUp(e) {
      if (drag === null) return
      const wasClick = !drag.moved && drag.onThumb
      const d = drag
      setDrag(null)
      if (wasClick && d.mode === 'move') {
        onExpand()
      }
    }
    ...
    const movingCls = drag !== null && drag.moved ? ' moving' : ''
    ...
    const view = state.expanded
      ...
      : h('div', { className: 'cvn-thumb', 'data-cvthumb': '1', title: '单击展开 · 按住拖动卡片' },
          state.html === ''
            ? h('div', { className: 'cvn-spin', style: { padding: '8px' } }, '载入中…')
            : h('iframe', { className: 'cvn-thumb-frame', srcDoc: state.html, sandbox: 'allow-scripts allow-popups' }))

    return h('div', {
      className: (state.expanded ? 'cvn-card cvn-card-big' : 'cvn-card') + movingCls,
      ...
      h('div', { className: 'cvn-foot' },
        ...
        state.out !== ''
          ? h('span', { className: 'cvn-link', title: state.out + '（点击在 Finder 中显示）', onClick: onReveal }, '📁 ' + state.msg + ' · 点击查看')
          : (state.msg !== ''
              ? h('span', { className: 'cvn-hint' }, state.msg)
              : h('span', { className: 'cvn-hint' }, state.expanded ? '按住卡片任意空白处拖动 · ◢ 调大小' : '点缩略图展开 · 按住卡片拖动'))),
  ...
  return {
    ...
    apply(ctx) {
      ...
      ctx.effect(function () {
        return function () { disposeOverlay(); disposeReg(); disposeCss(); pluginCtx = null }
      })
    },
  }
